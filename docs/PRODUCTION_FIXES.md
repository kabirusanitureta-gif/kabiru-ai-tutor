...
## 4. "Could not open requirements file: '../requirements.txt'" on Render

**Root cause:** `render.yaml`'s backend service originally set `rootDir: backend` combined with
`buildCommand: "pip install -r ../requirements.txt"`. This depends on Render building with a working
directory of exactly `backend/` so that `../requirements.txt` resolves to the repo root. If a person
sets up the service manually in the Render dashboard (rather than via the Blueprint) and leaves Root
Directory blank, or Render's build step otherwise runs from a different directory than expected, that
relative path breaks instantly with exactly this error.

**The fix:** the backend service in `render.yaml` no longer sets `rootDir` at all — it builds from the
repo root by default, where `requirements.txt` actually lives, using a plain `pip install -r
requirements.txt` with no relative-path guessing. The **start** command still `cd backend &&`s into
the backend folder, since that's where `app.main:app` lives — but build and start no longer depend on
matching some external Root Directory setting to agree with each other. This removes the entire class
of path-confusion bugs (the same category that caused earlier Railway deploy failures).

If you ever configure the backend manually instead of via the Blueprint, use exactly:
- **Root Directory:** *(leave blank — repo root)*
- **Build Command:** `pip install -r requirements.txt`
- **Start Command:** `cd backend && python -m app.seed.run_seed && uvicorn app.main:app --host 0.0.0.0 --port $PORT`

## 5. Alembic and SSL connection hardening

- `backend/alembic/env.py` previously built its own database URL directly from `settings.DATABASE_URL`,
  bypassing the `postgres://` → `postgresql://` scheme normalization already applied in
  `app/core/database.py`. Alembic now imports and reuses that same normalized `database_url`, so the
  running app and any manual `alembic upgrade` command always agree on exactly how to connect.
- `app/core/database.py` now automatically appends `sslmode=require` to non-SQLite connection strings
  that don't already specify one, since most hosted Postgres providers (Render, Railway, Supabase)
  require SSL for connections and this otherwise surfaces as an opaque, hard-to-diagnose connection
  failure on first deploy.
