...
def _assert_quiz_lesson_unlocked(db: Session, current_user: User, quiz: Quiz) -> None:
    lesson = db.query(Lesson).filter(Lesson.id == quiz.lesson_id, Lesson.is_deleted.is_(False)).first()
    if lesson:
        assert_lesson_unlocked(db, current_user, lesson)


def _attach_progress_to_quiz(db: Session, current_user: User, quiz: Quiz) -> Quiz:
    """Attach the user's saved pass/score state so the frontend can resume
    correctly instead of showing a blank quiz every time it's reopened."""
    progress = (
        db.query(Progress)
        .filter(Progress.user_id == current_user.id, Progress.lesson_id == quiz.lesson_id)
        .first()
    )
    quiz.quiz_passed = bool(progress and progress.quiz_passed)
    ...
    quiz.best_score_percent = best_attempt.score_percent if best_attempt else None
    return quiz
...
def get_quizzes_for_lesson(lesson_id: int, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    ...
    assert_lesson_unlocked(db, current_user, lesson)
    quizzes = db.query(Quiz).filter(Quiz.lesson_id == lesson_id, Quiz.is_deleted.is_(False)).all()
    return [_attach_progress_to_quiz(db, current_user, q) for q in quizzes]
...
def get_quiz(quiz_id: int, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    ...
    return _attach_progress_to_quiz(db, current_user, quiz)
...
def submit_quiz(quiz_id: int, payload: QuizSubmit, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    ...
    questions = db.query(Question).filter(Question.quiz_id == quiz_id, Question.is_deleted.is_(False)).all()
    if not questions:
