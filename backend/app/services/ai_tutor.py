...
Three-tier fallback chain, tried in order, so the app ALWAYS responds:

  1. Local Ollama (if OLLAMA_ENABLED and reachable) — free, for local dev
     on a machine with Ollama installed. Never available on Render.
  2. Groq cloud API (if AI_API_KEY is set) — free, permanent, no-credit-
     card tier (see docs/AI_PROVIDER.md for the full research behind this
     choice). This is what actually answers students on Render.
  3. Rule-based tutoring engine — zero external dependencies, zero paid
     APIs, always available. Final safety net if both of the above are
     down, unset, or rate-limited.
...
# ---------------------------------------------------------------------------
# Groq cloud fallback (free permanent tier — see docs/AI_PROVIDER.md)
# ---------------------------------------------------------------------------
def _ask_groq(prompt: str, language: str) -> str | None:
    """
    Calls Groq's OpenAI-compatible chat endpoint. Returns None (never raises)
    on any failure — missing key, timeout, rate limit, network error, or a
    malformed response — so the caller can transparently fall through to the
    rule-based tutor instead of showing the student a broken page.
    """
    ...
    try:
        ...
        # Groq is normally very fast; a timeout usually means a transient
        # network hiccup on Render's side. Fail soft, not loud.
        return None
    except httpx.RequestError:
        return None

    if resp.status_code == 429:
        # Free-tier rate limit hit (RPM/TPM/RPD). Don't retry inline — that
        # would just make the student wait longer for the same failure.
        # Falling through to the rule-based tutor keeps the response instant.
        return None
    if resp.status_code == 401:
        # Bad/missing/revoked API key — a config problem, not a traffic
        # problem. Still fail soft so the student gets *a* useful reply.
        return None
    if resp.status_code != 200:
        return None
...
def get_tutor_reply(message: str, preferred_language: str | None = None) -> tuple[str, str]:
    ...
    Fallback chain: local Ollama (dev only) -> Groq cloud API (production,
    free tier) -> rule-based tutor (always works, zero dependencies).
    """
    ...
    groq_reply = _ask_groq(message, language)
    if groq_reply:
        return groq_reply, language
