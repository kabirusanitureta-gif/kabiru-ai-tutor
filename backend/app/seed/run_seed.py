        ...
        for course_data in COURSES:
            ...
            for index, lesson_data in enumerate(course_data["lessons"], start=1):
                ...
                if quiz_data:
                    ...
                    # STABLE upsert: match existing questions to the new list by
                    # position (order) and UPDATE their fields in place, instead
                    # of deleting everything and re-inserting. This keeps
                    # Question IDs stable across repeated seed runs (which
                    # happen on every backend restart) so nothing appears to
                    # "reset" or "change" from a student's perspective — only
                    # genuinely new/removed questions actually change any rows.
                    existing_questions = (
                        db.query(Question)
                    ...
                    for idx, q in enumerate(new_questions):
                        ...
                        if idx < len(existing_questions):
                            # Update the existing row in place — same ID, synced content.
                            for field, value in fields.items():
                                setattr(existing_questions[idx], field, value)
                        else:
                            # Genuinely new question beyond the previous count.
                            db.add(Question(quiz_id=quiz.id, **fields))
                            total_questions_created += 1

                    # Only remove rows if the question list genuinely shrank.
                    if len(existing_questions) > len(new_questions):
                        for stale in existing_questions[len(new_questions):]:
                            db.delete(stale)
